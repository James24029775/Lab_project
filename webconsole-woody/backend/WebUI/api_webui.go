...
func GetTenantByID(c *gin.Context) {
	...
	filterTenantIdOnly := bson.M{"missionId": missionId}
	...
	var missionData Tenant
	json.Unmarshal(mapToByte(tenantDataInterface), &missionData)

	c.JSON(http.StatusOK, missionData)
}
...
func PostTenant(c *gin.Context) {
	...
	if missionData.MissionId == "" {
		missionData.MissionId = uuid.Must(uuid.NewRandom()).String()
	}

	tenantBsonM := toBsonM(missionData)
	filterTenantIdOnly := bson.M{"missionId": missionData.MissionId}
	if _, err := mongoapi.RestfulAPIPost(tenantDataColl, filterTenantIdOnly, tenantBsonM); err != nil {
		logger.WebUILog.Errorf("PostMission err: %+v", err)
	}
...
func PutTenantByID(c *gin.Context) {
	...
	tenantData.MissionId = missionId

	tenantBsonM := toBsonM(tenantData)
	filterTenantIdOnly = bson.M{"missionId": missionId}
	if _, err := mongoapi.RestfulAPIPost(tenantDataColl, filterTenantIdOnly, tenantBsonM); err != nil {
		logger.WebUILog.Errorf("PutMissionByID err: %+v", err)
	}
...
func DeleteTenantByID(c *gin.Context) {
	...
	if err := mongoapi.RestfulAPIDeleteMany(amDataColl, filterTenantIdOnly); err != nil {
		logger.WebUILog.Errorf("DeleteMissionByID err: %+v", err)
	}
...
// Utility function.
func GetTenantById(missionId string) map[string]interface{} {
	filterTenantIdOnly := bson.M{"missionId": missionId}
